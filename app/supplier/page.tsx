...
const productOptions = ["Rice", "Wheat", "Maize", "Mangoes", "Kinnow", "Onions", "Potatoes", "Sesame", "Garlic", "Dates", "Animal Feed", "Other"]

export default function SupplierPage() {
  const [form, setForm] = useState({ companyName: "", contactPerson: "", email: "", phone: "", country: "", products: [] as string[], exportCapacity: "", certifications: "", message: "" })
  const [loading, setLoading] = useState(false)
  const [success, setSuccess] = useState(false)
  const [error, setError] = useState("")

  const toggleProduct = (p: string) => {
    setForm(prev => ({ ...prev, products: prev.products.includes(p) ? prev.products.filter(x => x !== p) : [...prev.products, p] }))
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError("")
    try {
      const res = await fetch("/api/supplier", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, products: form.products.join(", ") }) })
      const data = await res.json()

      if (!res.ok) {
        throw new Error(data.error || "Failed to submit supplier profile")
      }

      setSuccess(true)
    } catch (error) {
      console.error("[supplier page] submit error:", error)
      setError(error instanceof Error ? error.message : "Failed to submit supplier profile")
    ...
    <>
      ...
      <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", padding:"2rem" }}>
        <motion.div initial={{ opacity:0, scale:.9 }} animate={{ opacity:1, scale:1 }} transition={{ duration:.6 }}
          ...
          <h2 style={{ fontFamily:"'Calibri', sans-serif", fontSize:"clamp(1.4rem,3.5vw,1.8rem)", fontWeight:400, color:"var(--textdk)", marginBottom:".5rem" }}>Supplier Profile Submitted!</h2>
          <p style={{ color:"var(--textmd)", fontSize:"clamp(.9rem,1.8vw,1rem)" }}>Our team will review your submission and contact you shortly.</p>
        </motion.div>
      </div>
    </>
  )
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .product-selector { display:flex; flex-wrap:wrap; gap:clamp(.5rem,1.5vw,.875rem); }
        .product-btn { padding:clamp(.5rem,1.5vw,.65rem) clamp(.875rem,2vw,1.1rem); border-radius:50px; font-size:clamp(.75rem,1.4vw,.8rem); font-weight:500; border:1.5px solid var(--border); background:#fff; color:var(--textmd); cursor:pointer; transition:all .25s ease; white-space:nowrap; }
        .product-btn.selected { background:var(--gold); color:#fff; border-color:var(--gold); }
      ...
      <section className="sec-pad" style={{ background:"var(--ivory)" }}>
        <div className="sec-wrap">
          <motion.div initial={{ opacity:0, y:30 }} animate={{ opacity:1, y:0 }} style={{ textAlign:"center", marginBottom:"clamp(2rem,5vw,3.5rem)" }}>
            <p className="pp-tag" style={{ marginBottom:".9rem" }}>Supplier Network</p>
            <h1 className="pp-section-heading" style={{ marginBottom:".9rem" }}>
              Become a <em style={{ fontStyle:"italic", color:"var(--green)" }}>Supplier</em>
            </h1>
            <span className="gold-rule-c" />
            <p className="pp-body" style={{ maxWidth:500, margin:"1rem auto 0", fontSize:".95rem" }}>
              Join our network of verified suppliers and access global markets.
            </p>
          </motion.div>
          ...
          <motion.div initial={{ opacity:0, y:20 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true, margin:"-60px" }} className="form-container">
            <form onSubmit={handleSubmit} style={{ display:"flex", flexDirection:"column", gap:"clamp(1.1rem,2.5vw,1.5rem)" }}>
              <div className="form-group">
                ...
                <input required className="form-input" value={form.companyName} onChange={e => setForm(p => ({ ...p, companyName: e.target.value }))} />
              </div>

              <div className="form-group">
                <label className="form-label">Contact Person / رابطہ کار کا نام *</label>
                <input required className="form-input" value={form.contactPerson} onChange={e => setForm(p => ({ ...p, contactPerson: e.target.value }))} />
              </div>
              ...
              <div className="form-row">
                ...
                <div className="form-group" style={{ marginBottom:0 }}>
                  ...
                  <input required className="form-input" placeholder="+92 300 1234567" value={form.phone} onChange={e => setForm(p => ({ ...p, phone: e.target.value }))} />
                </div>
              </div>
              ...
              <div className="form-group">
                <label className="form-label" style={{ marginBottom:"1rem" }}>Products You Supply / آپ کی فراہم کردہ مصنوعات *</label>
                <div className="product-selector">
                  {productOptions.map(p => (
                    <button type="button" key={p} onClick={() => toggleProduct(p)} className={`product-btn ${form.products.includes(p) ? "selected" : ""}`}>
                      {p}
                    </button>
                  ))}
                </div>
              ...
              <div className="form-group">
                <label className="form-label">Monthly Export Capacity / ماہانہ برآمدی صلاحیت *</label>
                <input required className="form-input" placeholder="e.g., 200 MT/month" value={form.exportCapacity} onChange={e => setForm(p => ({ ...p, exportCapacity: e.target.value }))} />
              </div>

              <div className="form-group">
                <label className="form-label">Certifications / سرٹیفکیٹس</label>
                <input className="form-input" placeholder="e.g., ISO, HACCP, Organic" value={form.certifications} onChange={e => setForm(p => ({ ...p, certifications: e.target.value }))} />
              </div>

              <div className="form-group">
                <label className="form-label">Additional Message / اضافی پیغام</label>
                <textarea rows={4} className="form-textarea" value={form.message} onChange={e => setForm(p => ({ ...p, message: e.target.value }))} />
              </div>

              <button type="submit" disabled={loading} className="submit-btn" style={{ marginTop:"clamp(.5rem,1vw,.75rem)" }}>
                {loading ? "Submitting..." : <>Submit Supplier Profile <Send style={{ width:14, height:14 }} /></>}
              </button>
